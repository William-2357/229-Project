...
class FoundationSourceFineTuneCLDAdapter(BaseAdapter):
    ...
    def fit(
        ...
        # =====================================================================
        # 🚀 FIX: LOSO-SAFE DISK-BASED PERSISTENT STORAGE FOR MODAL
        # =====================================================================
        import os
        
        # Create a unique hash of the source data arrays to dynamically identify 
        # which unique combination of background training subjects we are using.
        ...
        # Filename now includes the specific source split hash to prevent cross-subject leakage
        checkpoint_path = os.path.join(checkpoint_dir, f"{backbone_name}_seed{self.seed}_src_{src_hash}_sft.pt")
        
        if os.path.exists(checkpoint_path):
            print(f"📦 [Modal Volume] Found cached SFT weights for {backbone_name} (split: {src_hash}) at {checkpoint_path}!")
            print("🚀 Loading pre-baked weights instantly. Bypassing the 10-minute training loop!")
            
            cached_state = torch.load(checkpoint_path, map_location=self.device)
            model.load_state_dict(cached_state)
        else:
            print(f"❌ [Modal Volume] No cache file found at {checkpoint_path}")
            print(f"🔥 Commencing full 10-minute {backbone_name} pre-training loop over source dataset...")
            
            model = build_source_finetuned_foundation_model(
                self.backbone, n_classes, X_src, y_src, **self._source_ft_kwargs()
            )
            
            print(f"💾 Saving newly trained weights to network disk: {checkpoint_path}")
            torch.save(model.state_dict(), checkpoint_path)
            
            try:
                import __main__
                if hasattr(__main__, "data_volume"):
                    print("🔄 Committing changes to Modal Network Volume via __main__...")
                    __main__.data_volume.commit()
                    print("✅ Cache successfully locked into cloud volume for future runs!")
                else:
                    print("⚠️ Note: Weights saved locally. Ensure your experiment runner script calls data_volume.commit()!")
            except Exception as e:
                print(f"⚠️ Warning: Could not auto-commit volume from inside adapter: {e}")
        # =====================================================================
        ...
        if target_unlabeled is not None and len(target_unlabeled) >= 2:
            tgt_cache_key = ("foundation_sft_cld_tgt_feats", self.seed)
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        if self._cld_model is None or self._backbone_model is None:
            raise RuntimeError("FoundationSourceFineTuneCLDAdapter not fitted")
        X_feat = extract_foundation_features(
            self._backbone_model.to(self.device), X, self.device, self.batch_size
        )
