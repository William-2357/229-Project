...
def anchored_admm(model, admm_params, v_anchor=None, anchor_a: float = 0.0):
    ...
    Sets model.theta1/theta2 (the non-convex weights used by stacked_predict) and returns v.
    """
    ...
    a = float(anchor_a)
    if v_anchor is None or a == 0.0:
        v_anchor = jnp.zeros((C, 2, d, P))
        a = 0.0
    gamma_v = 1.0 / (rho + a)        # prox step for blended (rho + anchor) curvature
    ...
    for _ in range(admm_iters):
        ...
        # anchored v-update: blend ADMM point (u+lam) with the source anchor, then group-prox
        q0 = (rho * (u[:, 0, :] + lam[:, 0, :]) + a * v_anchor[:, 0, :]) / (rho + a)
        q1 = (rho * (u[:, 1, :] + lam[:, 1, :]) + a * v_anchor[:, 1, :]) / (rho + a)
        v = v.at[:, 0, :].set(batch_proxl2_tensor(q0, beta=beta, gamma=gamma_v))
        v = v.at[:, 1, :].set(batch_proxl2_tensor(q1, beta=beta, gamma=gamma_v))
