...
Use train_fit_time_warm for pure on-target training time -> {bb}_train_fit_time.png.
...
def main():
    ...
    for lab,fk,sk in GRAD:
        xs,ys=curve(nopad,{fk,sk},metric)
    ...
    for lab,fk,sk in CONV:
        ...
        xs2,ys2=curve(pad,{fk,sk},metric)
    ...
    out_path = OUT_DIR / f"{bb}_{metric.slug}fit_time.png"
    plt.tight_layout(); plt.savefig(out_path,dpi=150,bbox_inches="tight"); plt.close()
    print(f"Saved {out_path} [metric={metric.name}]")
